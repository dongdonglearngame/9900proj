...
review, the deterministic rules failed to flag any of the four review candidates (see Section~\ref{sec:review}). The result suggests keeping the
...
Each method retained its own search logic and, where used, proposer prompt. All 160
units completed in one job with no failures or skips. The set is success-enriched for
earlier Strategy~1 results, so
the comparison is within this selected development set, not a population estimate.
...
\bottomrule
\end{tabularx}
\caption{Common success-enriched dev-40 matrix. Plain \texttt{not\_found} excludes
diagnostic-only scenarios; ``Diag.'' counts diagnostic scenarios. Edit distance is
averaged over primary results, while calls and runtime are averaged over all 40 runs.}
\label{tab:commonmatrix}
\end{table}

The Baseline had the highest primary mechanical coverage on this set. Strategy~2 had
the smallest final edits, and Strategy~3 used the fewest target calls and had the
shortest runtime but separated six scenarios as diagnostic. The 35 primary outputs
still require human review. The Baseline's 0.03 proposer average is one shared polish
attempt; its search uses no proposer.

\subsection{Historical Strategy 1 Strict vs.\ Discovery}
\label{sec:s1results}

Before the common matrix, we ran Strategy~1's Sprint~2 strict version and
...
The Discovery version of Strategy~2 is implemented and covered by the 230-test backend
suite. In the common matrix it produced 6/40 primary review candidates, no diagnostic
scenarios, and 34 plain \texttt{not\_found} outcomes. Its primary results had an average
final edit distance of 1.00; all runs averaged 11.13 target calls, 6.03 proposer calls,
and 40.04~s. A paired strict-versus-Discovery evaluation was not completed.

\subsection{Runtime}
\label{sec:runtime}

In the common dev-40 matrix, all methods ran sequentially on the same RTX 4060 laptop
GPU. Average runtimes were 38.32~s for the Baseline, 25.06~s for Strategy~1, 40.04~s
for Strategy~2, and 13.48~s for Strategy~3. These values are comparable only within
that matrix.

Separately, dataset-wide runtimes are slow on CPU (400 questions take hours to
compute), so, following the client's suggestion, we present one high-runtime question:
...
run, while all 35 common-matrix primary results are still awaiting verification.
