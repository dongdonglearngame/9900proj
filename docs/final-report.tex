...
These four applied strategies differ in the way the structure of the modified unit is arranged; it may be a fixed vocabulary set, a free event-level rewrite, importance-driven span completion, and one concept-grounding procedure. Hence, we may use different search hypotheses rather than modifications of the same prompt. On the client's request, the report applies the names Strategies 1--3 and Baseline to refer to these strategies. Appendix~\ref{app:mapping} maps these terms to the corresponding identifiers in the code and in the experiment metadata. Figure~\ref{fig:flows} illustrates the search procedures for all these strategies.
...
the answer, or exceed the maximum edit size. The frozen target validates all of
...
is set to 0.60. The version retains up to three different validated flips and
...
\paragraph{\strattwo{}.}
This approach first determines the importance of each text span by occluding
one span at a time, querying the frozen target for the change in confidence in
...
has not been performed during the term; the expected benefit from using the former
approach has remained a design hypothesis (Section~4).
...
In the proposal, flipping and evaluation were two different processes. 
And Sprint 3 is a reflection of this approach. 
The process steps are done sequentially. 
The frozen target re-checks all the transformations that have been accepted. 
Only the selected primary flip goes into the pipeline, but not the diagnostic flip which is kept separately for sensitivity.
A step that simply scores, categorizes, or records the current text makes no further target call.

\paragraph{Minimality (greedy restoration).}
The minimiser receives an edited situation that has been validated and tries to minimise
...
distance. It should also maintain sentence breaks and not include option letters,
