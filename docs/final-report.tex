...
\begin{itemize}
  ...
  \item \textbf{Diagnostic finding:} a verified flip due to lexical, formatting,
...
(4.55 versus 1.95). Minimality reduced the edits in both versions by more than half.
...
applying the requested-count limit. From these, 64 unique candidates that passed the
deterministic primary filters and 63 target-verified proposals emerged; 14 of the
verified proposals flipped across 10 different scenarios. Structural rejections
included violations of sentence structure (94 proposals), and eight proposals
exceeded the changed-fraction limit. Review of these 10 candidates is still pending;
therefore, they remain mechanical flips for now.

\subsection{Sprint 3 Results: Strategy 3 Strict vs.\ Discovery}
\label{sec:s3results}

For Strategy~3, we froze a new evaluation set of 20 questions without considering its
scenarios or outputs. The evaluation set is disjoint from all previous Strategy~3 work
and all other reviewed IDs. We disclose that an initial Strategy~1 sweep once examined
all 400 EU questions; therefore, no set can be described as completely untouched. Both
