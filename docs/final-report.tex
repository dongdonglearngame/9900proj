...
Experiments and feedback from the client and tutor guided each design decision.
Table~\ref{tab:evolution} summarises the main changes made across the three
sprints.
...
\bottomrule
\end{tabularx}
\caption{Design changes from the Sprint 1 MVP to the completed Sprint 3 system.}
\label{tab:evolution}
\end{table}
...
\begin{itemize}
  \item \textbf{Foil.} The alternative answer that the user wants the model to
  give. The strategy can use the foil, but the target model cannot see it.
  \item \textbf{Mechanical flip.} A mechanical flip occurs when the frozen target
  model's answer after an edit matches the foil exactly. It establishes that the
  answer changed, but does not establish that the edit is a good explanation.
  \item \textbf{Minimality.} Minimality measures the size of an edit. We use the
  word-level edit distance between the original and edited scenarios, defined as the
  number of single-word insertions, deletions, and replacements required to transform
  one into the other. We also record the changed-word fraction by dividing this count
  by the scenario length in words. Smaller edits are easier to read and reason about.
  \item \textbf{Fluency.} Fluency measures whether the edited text still reads as
  natural English. A local judge model assigns the score
  (Section~\ref{sec:quality}), which we treat as a proxy rather than proof.
  \item \textbf{Semantic validity.} Semantic validity assesses whether the edit
  makes a meaningful change to an event, outcome, relationship, or behaviour in the
  situation. Invalid edits include copying the answer into the text, replacing a word
  with a synonym, or contradicting the rest of the story. Our code flags clear cases,
  while human reviewers make the final judgement by labelling each flip Good,
  ...
  our checks identify as a model sensitivity, such as a near-synonym swap or an
  answer leak. We retain it as evidence about the model but do not publish it as an
  explanation.
  \item \textbf{Budget.} The budget is the maximum number of target-model checks a
  strategy can use for one scenario. Search and post-processing have separate capped
  allowances. We record every check so that reported costs include all work.
\end{itemize}

\subsection{Frozen Target Harness}

The project explains a fixed black-box model. Every strategy uses the same target
prompt, decoding configuration, answer parser, and cache contract. Allowing strategies
to change the target prompt would make it impossible to tell whether an observed
difference came from the prompt or the edit. Appendix~\ref{app:prompts} gives the full
target prompt. We define a flip as
\texttt{prediction.answer == foil}.

\subsection{Strategy Families}
\label{sec:strategies}

The client requested clear strategy numbering, so the report refers to Strategies 1
to 3 and the Baseline. Appendix~\ref{app:mapping} maps these names to the identifiers
used in the code and stored experiment metadata. Figure~\ref{fig:flows} shows the
search process for each strategy.

\paragraph{Baseline: word-level greedy search.}
The Baseline tests small word and phrase substitutions from a fixed list and verifies
each attempt. It does not use an LLM to generate candidates. Its transparent,
inexpensive search provides a reference point, although it cannot express broader
changes to an event.

\paragraph{\stratone{}.}
A local proposer LLM reads the scenario, choices, and foil, then generates complete
rewrites that change one event, outcome, relationship, or observable behaviour.
Deterministic guards reject rewrites that break the structure, leak the answer, or
change too much. The frozen target verifies the remaining candidates. The Sprint 2
version (\emph{strict}) aims for about three changed words and permits at most six.
The Sprint 3 \emph{discovery} variant removes this fixed word cap but retains a
changed-fraction safety cap of 0.60. It keeps up to three distinct verified flips and
sends the best candidate to the shared quality pipeline. This design draws on the
zero-shot LLM counterfactual approach in \cite{bhattacharjee2024} while keeping the
target model and verification process fixed.

\paragraph{\strattwo{}.}
This strategy begins by estimating the importance of each part of the text. It
occludes one span at a time, asks the frozen target how the occlusion changes its
confidence in each answer, and ranks the spans by importance. When confidence values
are missing, the strategy uses an answer-change heuristic and reports the resulting
coverage instead of filling missing values with zeros. The proposer LLM then replaces
content in the highest-ranked spans, and the frozen target verifies each result. The
method adapts importance-guided contrastive editing \cite{ross2021} to a black-box
setting without gradients. The discovery variant divides the fixed budget between
importance probes and candidate checks, explores several high-ranked spans and mask
widths, and records diagnostic candidates separately.

\paragraph{\stratthree{}.}
This strategy changes one situational concept, such as time, place, relationship,
urgency, or outcome. The proposer LLM identifies an exact span and supplies its
replacement. Our code verifies that the span occurs once and applies the replacement,
which prevents the LLM from rewriting the full story. The method draws on concept-level
counterfactual work \cite{gat2024}. We call it \emph{causal-inspired} because changing
one concept and observing the model constitutes an intervention on the text. It does
not establish real-world causality. The discovery variant adds an outcome concept
class, permits a coherent multi-word replacement span, and classifies overbroad spans
(at least 12 tokens and more than half of the scenario) as diagnostic findings.
...
which are the only mechanism available for testing a candidate. Each verified flip
then passes through the shared quality stages in the bottom row. Every stage uses the
same frozen target for re-verification.}
\label{fig:flows}
\end{figure}
...
Sprint 3 implements this design. The stages run in a fixed order, and the frozen
target re-verifies every accepted transformation.

\paragraph{Minimality (greedy restoration).}
The minimiser starts with a verified edited scenario and attempts to reduce the edit.
It aligns the original and edited text, identifies the changed regions, and restores
original content in stages, first by clause and then by word. After each proposed
restoration, it queries the frozen target. The minimiser keeps the restoration if the
answer remains the foil and reverses it if the flip is lost. It stops when it cannot
restore more content or reaches the post-processing call cap. This procedure produces
a verified local minimum, but does not guarantee a global minimum. We record the edit
distance before and after minimisation and retain both texts. A smaller final edit
does not by itself establish that the original edit captured a sound explanation.

\paragraph{Fluency (scored proxy).}
A separate local judge uses the same \texttt{llama3.2:3b} model with a fixed rubric
prompt, temperature 0, and prompt version \texttt{fluency-judge-v2-rubric-100}. It
reads only the edited scenario, without the choices or foil, and returns an integer
from 0 to 100: 90 to 100 is natural, 75 to 89 has a minor issue, 40 to 74 is
noticeably awkward, and 0 to 39 is broken. We divide this integer by 100 to obtain
$\text{score} = \text{judge integer} / 100 \in [0,1]$. A result passes when
$\text{score} \ge 0.75$. If the judge fails or produces invalid output, we record
\texttt{not\_measured} and the reason rather than inventing a value. The score remains
a proxy. Section~\ref{sec:qualitative} documents a broken sentence that passed, so a
fluency pass does not prove grammaticality.

\paragraph{One constrained polish.}
When the fluency score is low, the pipeline may make one polish attempt. The operation
can only delete unnecessary words from the changed span while preserving the order of
the remaining words. It cannot add, replace, reorder, or inflect words; increase the
edit distance; alter sentence boundaries; or introduce option letters, answer wording,
or emotion labels. The pipeline keeps the polished text only if its score increases
and it still flips the frozen target. Otherwise, the pipeline restores the pre-polish
result.

\paragraph{Risk flags and human review.}
Deterministic checks identify high-confidence problems, including exact or
morphological answer leakage, option references such as ``answer is C'', changes that
only replace a near-synonym or surface form, focal-person shifts, multi-concept spans,
and overbroad concept spans. We record flagged flips as diagnostic findings. All other
flips remain ``mechanical flip, review needed'' until two reviewers independently
label each one Good, Acceptable, or Invalid and assign reason codes. The reviewers
cannot see each other's labels, and an adjudicator resolves disagreements. Minimality
and fluency cannot make an invalid edit valid. A short, grammatical answer leak remains
invalid.
